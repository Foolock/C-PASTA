...
class Pin {
  ...
  private:
    ...
    // to initialize local critical path cost for each pins, i know it is too much... need to change later...
    bool _fvisited = false;
    bool _bvisited = false;
    int _fself_cost = 10;
    int _bself_cost = 1;
    int _ftemp_cost_prev_self = 0; // temporary prev local critical path cost + self cost of ftask
    int _ftemp_cost_after_self = 0; // temporary after local critical path cost + self cost of ftask
    int _btemp_cost_after_self = 0; // temporary prev local critical path cost + self cost of btask
    int _btemp_cost_prev_self = 0; // temporary abter local critical path cost + self cost of btask
    int _flocal_crit_cost = 0; // local_crit_cost = prev_crit_cost + self_cost + after_crit_cost of ftask
    int _blocal_crit_cost = 0; // local_crit_cost = prev_crit_cost + self_cost + after_crit_cost of btask
