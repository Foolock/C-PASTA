...
void Timer::_update_timing() {
  ...
  // partition vivekDAG 
  _partition_vivekDAG_GDCA_cpu_async();
...
void Timer::_GDCA_build_coarsen_graph() {
  ...
  */
  ...
  std::cout << "dependency amount: " << dep_cnt << "\n";
}
...
void Timer::_partition_vivekDAG_GDCA_cpu_async() {
  ...
  for(auto vtask : _vivekDAG._vtask_ptrs) { // each source task starts with its own partition
    if(vtask->_fanin.size() == 0) { 
      ...
      vtask->_ptftask = _executor.silent_dependent_async([](){});
    }
  }
  ...
  for(auto vtask : _vivekDAG._vtask_ptrs) {
    if(vtask->_fanin.size() != 0) {
      ...
      for(auto dependent_id : vtask->_fanin) {
        dependent_tasks.push_back(_vivekDAG._vtask_ptrs[dependent_id]->_ptftask);
      }
      vtask->_ptftask = _executor.silent_dependent_async([this, vtask, &max_cluster_id, &cluster_cnt](){
  ...
  */
  int cur_cluster_id = -1;
  std::queue<VivekTask*> ready_queue;
  std::atomic<size_t> task_cnt = 0;
  std::vector<std::atomic<size_t>> fu_partition(_vivekDAG._vtask_ptrs.size());
  for(size_t i=0; i<_vivekDAG._vtask_ptrs.size(); i++) {
    fu_partition[i] = 0;
  }
  for(auto vtask : _vivekDAG._vtask_ptrs) { // each source task starts with its own partition
  ...
  while(task_cnt < _vivekDAG._vtask_ptrs.size()) {
    // std::cout << "looping.\n";
    ...
    VivekTask* cur_task;
    {
      ...
      cur_task = ready_queue.front();
      ready_queue.pop();
    }
    task_cnt++;
    if(cur_task) {
      cur_task->_ptftask = _executor.silent_dependent_async([this, cur_task, &max_cluster_id, &cluster_cnt, &task_cnt, &ready_queue, &mtx](){
        // release neighbor's dependents and assign desired_partition id
        for(auto successor_id : cur_task->_fanout) {
          if(_vivekDAG._vtask_ptrs[successor_id]->_pins[0].second->_dep_cnt.fetch_add(1) == _vivekDAG._vtask_ptrs[successor_id]->_fanin.size()-1) {

            std::vector<int> dependent_cluster_ids;
            for(auto dependent_id : _vivekDAG._vtask_ptrs[successor_id]->_fanin) {
              dependent_cluster_ids.push_back(_vivekDAG._vtask_ptrs[dependent_id]->_cluster_id);
            }
            int desired_cluster_id =  *(std::max_element(dependent_cluster_ids.begin(), dependent_cluster_ids.end()));
            ...
            // check if that partition is full
            if(cluster_cnt[desired_cluster_id].fetch_add(1) < partition_size) {
              _vivekDAG._vtask_ptrs[successor_id]->_cluster_id = desired_cluster_id;
            }
            else {
              int new_cluster_id = max_cluster_id.fetch_add(1) + 1;
              _vivekDAG._vtask_ptrs[successor_id]->_cluster_id = new_cluster_id;
              cluster_cnt[new_cluster_id]++;
            }
            {
              std::scoped_lock lock(mtx);
              ready_queue.push(_vivekDAG._vtask_ptrs[successor_id]);
            }
          }
        }
      }); 
