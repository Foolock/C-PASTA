...
void Timer::_update_timing() {
  ...
  // partition vivekDAG 
  _partition_vivekDAG_GDCA_origin();
...
//
  ...
  */
  ...
  if(circuit_duplication == 1) {
    return;
  }
  else { // connect extra duplication circuit
    std::vector<std::vector<int>> source(circuit_duplication); // source_sink[i] is the sources of DAG i 
    std::vector<std::vector<int>> sink(circuit_duplication); // source_sink[i] is the sinks of DAG i 
    int total_DAG_size = _vivekDAG._vtask_ptrs.size();  
    int one_DAG_size = total_DAG_size / circuit_duplication; // hardcoded circuit size 
    for(int DAG_id=0; DAG_id<circuit_duplication; DAG_id++) { // the DAG we are processing
      for(int offset=0; offset<one_DAG_size; offset++) { // the offset of the task in that DAg
        int task_id = DAG_id * one_DAG_size + offset;
        if(_vivekDAG._vtask_ptrs[task_id]->_fanin.size() == 0) {
          source[DAG_id].push_back(task_id);
        }
        if(_vivekDAG._vtask_ptrs[task_id]->_fanout.size() == 0) {
          sink[DAG_id].push_back(task_id);
        }
      }
    }
    // for(int DAG_id=0; DAG_id<circuit_duplication; DAG_id++) { // the DAG we are processing
    //   std::cout << "DAG_id = " << DAG_id << "\n";
    //   std::cout << "source = [";
    //   for(auto source_id : source[DAG_id]) {
    //     std::cout << source_id << " ";
    //   }
    //   std::cout << "]\n";
    //   std::cout << "sink = [";
    //   for(auto sink_id : sink[DAG_id]) {
    //     std::cout << sink_id << " ";
    //   }
    //   std::cout << "]\n";
    // }
    // for each sinks in DAG i, connect it to all the sources in DAG i+1
    int diff = sink[0].size() - source[0].size();
    if(diff >= 0) { // num_sink >= num_source
      for(int DAG_id=0; DAG_id<circuit_duplication-1; DAG_id++) {
        for(unsigned id=0; id<source[0].size(); id++) {
          _vivekDAG._vtask_ptrs[sink[DAG_id][id]]->addFanout(source[DAG_id+1][id]);
        }
      }
    }
    else { // num_sink < num_source
      for(int DAG_id=0; DAG_id<circuit_duplication-1; DAG_id++) {
        for(unsigned id=0; id<sink[0].size(); id++) {
          _vivekDAG._vtask_ptrs[sink[DAG_id][id]]->addFanout(source[DAG_id+1][id]);
        }
      }
    }
...
void Timer::_export_csr() {
  ...
  std::cout << "the size of _adjncy is " << _adjncy.size() << "\n";
...
void Timer::_GDCA_dfs() {
  ...
  auto end = std::chrono::steady_clock::now();
  GDCA_par_runtime += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();
...
void Timer::_GDCA_build_coarsen_graph() {
  ...
  */
  ...
  int dep_cnt = 0;
  for(size_t i=0; i<_rebuild_vivekDAG._vtask_ptrs.size(); i++) {
    dep_cnt += _rebuild_vivekDAG._vtask_ptrs[i]->_fanout.size();
  }
}
...
void Timer::_partition_vivekDAG_GDCA_cpu() {
...
void Timer::_partition_vivekDAG_GDCA_gpu_deterministic() {
...
void Timer::_partition_vivekDAG_GDCA_cpu_async() {

  // clear taskflow
  _taskflow.clear();
  /*
   * step1. get topological order, this method won't make sense cuz it needs topological order first
   */
  // check if _vivekDAG._vtask_ptrs follows topological order
  // for(auto vtask : _vivekDAG._vtask_ptrs) {
  //   std::cout << "pin: " << vtask->_pins[0].second->_name << "\n";
  // }

  /*
   * step2. build async task according to topological order
   */
  auto start = std::chrono::steady_clock::now();
  int cur_cluster_id = -1;
  for(auto vtask : _vivekDAG._vtask_ptrs) { // each source task starts with its own partition
    if(vtask->_fanin.size() == 0) { 
      cur_cluster_id++;
      vtask->_cluster_id = cur_cluster_id;
      // empty task for source task
      vtask->_ptftask_async = _executor.silent_dependent_async([](){});
    }
  }
  std::atomic<int> max_cluster_id = cur_cluster_id;
  std::vector<std::atomic<int>> cluster_cnt(_vivekDAG._vtask_ptrs.size());
  for(int i=0; i<=cur_cluster_id; i++) {
    cluster_cnt[i]++;
  }
  for(auto vtask : _vivekDAG._vtask_ptrs) {
    if(vtask->_fanin.size() != 0) {
      tf::SmallVector<tf::AsyncTask> dependent_tasks;
      for(auto dependent_id : vtask->_fanin) {
        dependent_tasks.emplace_back(_vivekDAG._vtask_ptrs[dependent_id]->_ptftask_async);
      }
      vtask->_ptftask_async = _executor.silent_dependent_async([this, vtask, &max_cluster_id, &cluster_cnt](){
        // get desired partition id
        std::vector<int> dependent_cluster_ids;
        for(auto dependent_id : vtask->_fanin) {
          dependent_cluster_ids.push_back(_vivekDAG._vtask_ptrs[dependent_id]->_cluster_id);
        }
        int desired_cluster_id =  *(std::max_element(dependent_cluster_ids.begin(), dependent_cluster_ids.end()));

        // check if that partition is full
        if(cluster_cnt[desired_cluster_id].fetch_add(1) < partition_size) {
          vtask->_cluster_id = desired_cluster_id;
        }
        else {
          int new_cluster_id = max_cluster_id.fetch_add(1) + 1;
          vtask->_cluster_id = new_cluster_id;
          cluster_cnt[new_cluster_id]++;
        }
      }, dependent_tasks.begin(), dependent_tasks.end()); 
    }
  } 
  
  /*
   * step3. wait for all tasks to finish
   */
  _executor.wait_for_all();
  auto end = std::chrono::steady_clock::now();
  CPU_Async_par_runtime += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();
  _total_num_partitions_cpu = max_cluster_id + 1;

  /*
   * step4. rebuild partitioned taskflow
   */
  _GDCA_build_coarsen_graph_cpu_async();
...
void Timer::_partition_vivekDAG_GDCA_cpu_taskflow() {
  ...
  for(auto vtask : _vivekDAG._vtask_ptrs) { // each source task starts with its own partition
    if(vtask->_fanin.size() == 0) { 
      cur_cluster_id++;
      vtask->_cluster_id = cur_cluster_id;
      vtask->_ptftask = _taskflow.emplace([](){});
    }
  }
  std::atomic<int> max_cluster_id = cur_cluster_id;
  std::vector<std::atomic<int>> cluster_cnt(_vivekDAG._vtask_ptrs.size());
  ...
  for(auto vtask : _vivekDAG._vtask_ptrs) {
    if(vtask->_fanin.size() != 0) {
      vtask->_ptftask = _taskflow.emplace([this, vtask, &max_cluster_id, &cluster_cnt](){
        // get desired partition id
        // std::vector<int> dependent_cluster_ids;
        // for(auto dependent_id : vtask->_fanin) {
        //   dependent_cluster_ids.push_back(_vivekDAG._vtask_ptrs[dependent_id]->_cluster_id);
        // }
        // int desired_cluster_id =  *(std::max_element(dependent_cluster_ids.begin(), dependent_cluster_ids.end()));
        int desired_cluster_id =  -1;
        for(auto dependent_id : vtask->_fanin) {
          if(_vivekDAG._vtask_ptrs[dependent_id]->_cluster_id > desired_cluster_id) {
            desired_cluster_id = _vivekDAG._vtask_ptrs[dependent_id]->_cluster_id;
          }
        }
  ...
  for(size_t task_id=0; task_id<_vivekDAG._vtask_ptrs.size(); task_id++) {
    for(auto successor_id : _vivekDAG._vtask_ptrs[task_id]->_fanout) {
      _vivekDAG._vtask_ptrs[task_id]->_ptftask.precede(_vivekDAG._vtask_ptrs[successor_id]->_ptftask);
    }
  }
  ...
  _executor.run(_taskflow).wait();
  auto end = std::chrono::steady_clock::now();
  CPU_Taskflow_par_runtime += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();
  _taskflow.clear();
  _total_num_partitions_cpu = max_cluster_id + 1;
...
void Timer::_partition_vivekDAG_GDCA_cpu_threadpool() {

  threadpool_partition();
...
void Timer::threadpool_partition() {

  size_t num_threads = std::thread::hardware_concurrency();
  std::vector<std::thread> threads;
  std::vector<WorkStealingQueue<int>> queues(num_threads); // work stealing queue to store task id in vivekDAG 
  ...
  for(auto vtask : _vivekDAG._vtask_ptrs) { // each source task starts with its own partition
    if(vtask->_fanin.size() == 0) {
      ...
      vtask->_cluster_id = cur_cluster_id;
      ++queue_index;
      queues[0].push(vtask->_id);
      // queues[queue_index%num_threads].push(vtask->_id);
    }
  }
  std::atomic<int> max_cluster_id = cur_cluster_id;
  std::vector<std::atomic<int>> cluster_cnt(_vivekDAG._vtask_ptrs.size());
  for(int i=0; i<_vivekDAG._vtask_ptrs.size(); i++) {
    cluster_cnt[i] = 0;
  }
  ...
  for(size_t i=0; i<num_threads; i++) {
    threads.emplace_back([this, i, &cluster_cnt, &max_cluster_id, &task_cnt, &queues, &num_threads](){
      // bar.arrive_and_wait();
      while(task_cnt.load(std::memory_order_relaxed) < _vivekDAG._vtask_ptrs.size()) {
        // check if there is any task left in its own queue
        std::optional<int> task_id;
        int desired_cluster_id;
        while(!queues[i].empty()) {
          task_id = queues[i].pop();
          if(task_id.has_value()) {
            // do the task
            VivekTask* vtask = _vivekDAG._vtask_ptrs[task_id.value()];
            // task_cnt.fetch_add(1);
            task_cnt.fetch_add(1, std::memory_order_relaxed);
            desired_cluster_id = vtask->_cluster_id; // cluster_id is initialized as -1(excluding source tasks)
            for(auto dependent_id : vtask->_fanin) {
              if(_vivekDAG._vtask_ptrs[dependent_id]->_cluster_id > desired_cluster_id) {
                desired_cluster_id = _vivekDAG._vtask_ptrs[dependent_id]->_cluster_id;
              }
            }
            if(cluster_cnt[desired_cluster_id].fetch_add(1, std::memory_order_relaxed) < partition_size) { 
              vtask->_cluster_id = desired_cluster_id;
            }
            else {
            ...
            // release dependents for its neighbor 
            while(vtask->_fanout.size() == 1) { // if it has only one neighbor, no need to release dependent
              if(_vivekDAG._vtask_ptrs[vtask->_fanout[0]]->_fanin.size() != 1) { // check linear chain
                break; 
              }
              // task_cnt.fetch_add(1);
              task_cnt.fetch_add(1, std::memory_order_relaxed);
              // assign partition id for this neighbor
              desired_cluster_id = vtask->_cluster_id;
              vtask = _vivekDAG._vtask_ptrs[vtask->_fanout[0]]; 
              if(cluster_cnt[desired_cluster_id].fetch_add(1, std::memory_order_relaxed) < partition_size) {
                vtask->_cluster_id = desired_cluster_id;
              }
              else {
            ...
            for(auto successor_id : vtask->_fanout) { // if its has more than one neighbor, release their dependents atomically
              if(_vivekDAG._vtask_ptrs[successor_id]->_pins[0].second->_dep_cnt.fetch_add(1, std::memory_order_relaxed) == _vivekDAG._vtask_ptrs[successor_id]->_fanin.size()-1) {
                // assign partition id for this neighbor
                queues[i].push(successor_id);
              }
            }
          }
        }
  ...
  auto end = std::chrono::steady_clock::now();
  CPU_Threadpool_par_runtime += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();

  _total_num_partitions_cpu = max_cluster_id + 1;
} 

void Timer::_GDCA_build_coarsen_graph_cpu_async() {

  // assigned cluster_id of each vtask in _vivekDAG to _partition_result_cpu
  _partition_result_cpu.resize(_vivekDAG._vtask_ptrs.size(), -1);  
  for(size_t task_id=0; task_id<_partition_result_cpu.size(); task_id++) {
    _partition_result_cpu[task_id] = _vivekDAG._vtask_ptrs[task_id]->_cluster_id;
  }

  // traverse _topo_result_cpu, assigned _vivekDAG._vtask_clusters -> such as to ensure each cluster has topological order
  // _vivekDAG._vtask_ptrs has maintained the topological order
...
//  std::cerr << _vivekDAG._vtask_ptrs.size() << "\n";
  ...
  _total_num_partitions_cpu = 0;
  _total_num_partitions_gpu = 0;
