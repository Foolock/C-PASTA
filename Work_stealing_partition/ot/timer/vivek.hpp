...
namespace ot {
  ...
  class VivekTask {
    ...
    private:
      int _id;
      int _cluster_id = -1;
      tf::Task _tftask; // task for update_timing
      tf::AsyncTask _ptftask_async; // task for partition
      tf::Task _ptftask; // task for partition
      size_t _runtime; 
      ...
      // dep_cnt for async task
      // std::atomic<size_t> _dep_cnt{0};
      bool _wsq_pushed = false;
  };
